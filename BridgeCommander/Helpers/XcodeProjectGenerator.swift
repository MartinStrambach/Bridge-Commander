...
//  Bridge Commander
//
//  Helper for generating Xcode projects using ti and tg commands
//
...
struct XcodeProjectGenerator {
    ...
    @MainActor
    static func generateProject(at repositoryPath: String, onStateChange: @escaping (XcodeProjectState) -> Void) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            process.terminationHandler = { process in
                if process.terminationStatus == 0 {
                    ...
                    continuation.resume(throwing: ProjectGenerationError.commandFailed(command: command, message: errorMessage))
                }
            }
            ...
            do {
                ...
                continuation.resume(throwing: ProjectGenerationError.commandFailed(command: command, message: error.localizedDescription))
            }
        }
    }
}
...
enum ProjectGenerationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .commandFailed(let command, let message):
            return "Command '\(command)' failed: \(message)"
        case .projectNotFoundAfterGeneration:
            return "Project not found after generation"
        case .failedToOpenProject:
            return "Failed to open Xcode project"
        case .iosFlashscoreFolderNotFound:
            return "ios/Flashscore folder not found in repository"
        }
    }
}
