...
- Keyboard focus between the commit list and the file list is an explicit `@FocusState` (`GitGraphPane`), not left to clicks: with a plain `Bool` the commit list kept focus after a file was clicked, so ↑/↓ kept walking the commits. Picking a different file moves focus to the file list (only a real change — the list also writes its selection back on reload, and moving focus on those writes stole it from the commit list after every ↑/↓); selecting a commit (click or key) moves it back to the commit list — the file list is swapped for a spinner while the new commit loads, so leaving focus there left ↑/↓ reaching nothing; → (commit list, with the pane open) and ← (file list) switch between them
