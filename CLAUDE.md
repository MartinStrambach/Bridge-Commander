...
- The font is assigned in the same place but **only when the point size actually differs**, unlike the flags beside it. SwiftTerm's `font` setter rebuilds the bold/italic faces, calls `selectNone()`, and re-derives cols/rows from the new cell size in `resetFont()` — which resizes the PTY and SIGWINCHes the shell. That is correct for a deliberate zoom and wrong on every unrelated update pass, which is why the assignment is guarded
